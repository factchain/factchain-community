...
export const POST_URL_REGEX =
  /^(https:\/\/(twitter|x).com\/.+?\/status\/\d+).*$/;
export const NOTE_URL_REGEX =
  /^(https:\/\/(twitter|x).com\/.+?\/birdwatch\/.+?\/\d+).*$/;
/*
...
*/
export const sanitizeXUrl = (url) =>
  url ? url.replace(/^(http(s)?):\/\/twitter.com/, '$1://x.com') : url;
...
export const FC_SFT_CONTRACT_ABIT = [
  {
      "type": "constructor",
      "inputs": [
          {
  ...
  {
      "type": "event",
      "name": "FactchainBuildersRewarded",
      "inputs": [
          {
              "name": "amount",
              "type": "uint256",

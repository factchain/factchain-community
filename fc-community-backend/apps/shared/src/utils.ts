...
export function time_period_to_block_periods(
  ...
  const blocksBetweenFromAndTo =  Math.ceil(timeDeltaToFromSeconds / ethAvgBlockTime)
  const blocksRange = Math.min(
    blocksBetweenFromAndTo,
    maxBlockDifference,
  );

  const dateNow = dateNowFixed ? dateNowFixed : new Date();
  const timeDeltaSeconds = Math.ceil((Math.abs(dateNow.getTime() - to.getTime()) / 1000));
  const toBlocksNumber = currentBlock - Math.ceil(timeDeltaSeconds / ethAvgBlockTime);
  ...
  for (
    ...
    const endBlock = startBlock + Math.min(blocksRange, toBlocksNumber - startBlock);

...
class FactchainContract {
  ...
  constructor(address: `0x${string}`, abi: any[]) {
    ...
    this.abi = abi;
    this.events = this.abi.filter((abi) => abi.type === 'event').reduce((acc, event) => {
      acc.set(event.name, event.inputs.map((input: any) => input.name));
      return acc;
  ...
  parseEvent(event: EventLog) {
    ...
    if (eventArgsNames) {
      ...
      throw new Error(`Event ${event.eventName} not found in contract ${this.address}`);
    }
  }
}
...
export const getEventsForNetwork = async (networkName: string, fromBlock: number) => {
  const network = supportedNetworks.find((network) => network.name === networkName);
  if (!network) {
  ...
  while (currentBlock < toBlock) {
    ...
    events.push(...(await getEvents(network, provider, currentBlock, nextBlock)));
    currentBlock = nextBlock;
  }
  return events;
};

const getEvents = async (network: any, provider: ethers.JsonRpcProvider, fromBlock: number, toBlock: number) => {
  console.log(`Getting ${network.name} events from block ${fromBlock} to block ${toBlock}`);
  // keep track of block timestamps to avoid querying the same block multiple times
  ...
  const allLogs = await Promise.all(
    network.contracts.map(async (factchainContract: FactchainContract) => {
      const contract = new ethers.Contract(factchainContract.address, factchainContract.abi, provider);
        ...
        events.map(async (event: Log | EventLog) => {
          if (event instanceof EventLog) {
            if (!blockTimestamps[event.blockHash]) {
              blockTimestamps[event.blockHash] = (await event.getBlock()).timestamp.toString();
            }
            return {
              ...
              args: factchainContract.parseEvent(event),
            }
          } else {
            return null;
          }
